...
echo ""
echo "4. Checking if resource limits were added by mutation policy..."
kubectl get pod test-valid-pod -o jsonpath='{.spec.containers[0].resources}' | jq .
...
echo "  ✓ add-default-resources: Mutates pods to add resource limits"
